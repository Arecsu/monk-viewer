...
#define m_pi 3.14159265359;           /* MathConstant: PI             */            
#define m_2_pi 6.2831853071; 
#define m_i_pi 0.31830988618;           /* MathConstant: 1 / PI         */
#define M_SQURT_2 1.41421356237;           /* MathConstant: sqrt(2)        */
#define M_I_SQURT_2 0.70710678119;           /* MathConstant: 1/sqrt(2)      */
...
float p22_beckmann_anisotropic(float x, float y, float alpha_x, float alpha_y)
{
    float x_sqr = x*x;
    float y_sqr = y*y;
    float sigma_x = alpha_x * M_I_SQURT_2;
    float sigma_y = alpha_y * M_I_SQURT_2;
...
void specular_anti_aliasing(in vec3 halfVector, 
                            inout float roughness) {
    float sigma = 0.90; // Screen-space variance
    float Kappa = 0.18; // Clamping threshold
    vec2 H = halfVector.xy; // Projected half-vector onto the screen
    vec2 footprintBoundingBox = fwidth(H); // Screen-space derivatives
    vec2 variance = sigma * sigma * footprintBoundingBox * footprintBoundingBox;
    vec2 kernelRoughness = min(vec2(Kappa), 2.0 * variance); 
    float adjustedRoughness = sqrt(roughness * roughness + kernelRoughness.x);
    roughness = adjustedRoughness; // Update roughness
}
...
float perspectiveDepthToViewZ( const in float depth, const in float near, const in float far ) {
  ...
  vec3 getIBLRadiance(const in vec3 viewDir, const in vec3 normal, const in float roughness) {
      ...
      specular_anti_aliasing(reflectVec, adjustedRoughness);
    ...
    vec3 getIBLAnisotropyRadiance(const in vec3 viewDir, const in vec3 normal, const in float roughness, const in vec3 bitangent, const in float anisotropy) {
        ...
        // Apply specular anti-aliasing
        float adjustedRoughness = roughness;
        specular_anti_aliasing(bentNormal, adjustedRoughness);

        return getIBLRadiance(viewDir, bentNormal, adjustedRoughness);
      #else
        return vec3(0.0);
      #endif
    }
...
float D_GGX( const in float alpha, const in float dotNH ) {
  ...
  vec3 BRDF_GGX_Clearcoat( const in vec3 lightDir, const in vec3 viewDir, const in vec3 normal, const in PhysicalMaterial material) {
    ...
    specular_anti_aliasing(halfDir, roughness);
...
vec3 BRDF_Sheen( const in vec3 lightDir, const in vec3 viewDir, const in vec3 normal, vec3 sheenColor, const in float sheenRoughness ) {
  specular_anti_aliasing(halfDir, sheenRoughness);
  vec3 halfDir = normalize( lightDir + viewDir );
  float dotNL = saturate( dot( normal, lightDir ) );
  float dotNV = saturate( dot( normal, viewDir ) );
  float dotNH = saturate( dot( normal, halfDir ) );
...
vec3  F = fresnel_schlick(F0, 1.0, wi_dot_wh);
...
vec3 diffuse_lambert = M_1_PI * (1.0 - metalnessFactor) * base_color;

// DIFFUSE_LAMBERT IS BRDF_Lambert

float lighting = world_get_shadow(P + EPSILON3F * N, L, 2.0) * wg_dot_wi * world_get_light_intensity();
...
- alpha_x, alpha_y (seems to be alpha roughness)
- wo = geometryViewDir
- wi = directLight.direction
- wh = normalize( wo + wi )
- wi_dot_wh = clamp( dot ( wi, wh ), 0., 1. );
- specular anti aliasing is a void so it's going to modify alpha_x and alpha_y after calling it

- we don't need to call to diffuse_lambert in our custom function. That would be BRDF_Lambert and it is already being called

vec3 wh = normalize( geometryViewDir + directLight.direction )
float wi_dot_wh = clamp(dot(directLight.direction, wh), 0., 1.)
float lambda_wo = lambda_beckmann_anisotropic(geometryViewDir, alpha_x, alpha_y);
float lambda_wi = lambda_beckmann_anisotropic(directLight.direction, alpha_x, alpha_y);
float D = ndf_beckmann_anisotropic(wh, alpha_x, alpha_y); 
float G = 1.0 / (1.0 + lambda_wo + lambda_wi);
vec3  F = fresnel_schlick(material.clearcoatF0, 1.0, wi_dot_wh);
